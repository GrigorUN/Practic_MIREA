                    ...
                    if i == 0: # Если это первый столбец (индекс базисной переменной)
                        ...
                        if j == 0 and start is False:  # Если это первая строка и start равно False
                            if round(tableau[j, i], 2) == 0:
                                print(round(tableau[j, i], 2), end="\t ")  # Если значение округленное до 2 знаков после запятой равно 0
                            else:
                                print((-1) * round(tableau[j, i], 2), end="\t ")  # В противном случае, печать отрицательного значения
                        else:
